...
public class EmailService {
    ...
    public void sendInvitationResponseConfirmation(
            ...
            boolean isAccepted,
            String viewToken) {
        sendInvitationResponseConfirmation(
            inviteeEmail, inviteeName, meetingTitle, meetingStartTime, meetingEndTime,
            roomName, roomLocation, inviterName, isAccepted, viewToken, null);
    }
    
    /**
     * Gửi email xác nhận kết quả phản hồi lời mời cho người được mời với file .ics đính kèm (khi chấp nhận)
     * @param inviteeEmail Email của người được mời
     * @param inviteeName Tên của người được mời (có thể là email nếu không có tên)
     * @param meetingTitle Tiêu đề cuộc họp
     * @param meetingStartTime Thời gian bắt đầu cuộc họp
     ...
     * @param inviterName Tên người mời
     * @param isAccepted true nếu đồng ý, false nếu từ chối
     * @param viewToken Token để xem chi tiết cuộc họp
     * @param meetingResponse MeetingResponse object để tạo file .ics (có thể null)
     */
    public void sendInvitationResponseConfirmation(
            String inviteeEmail, 
            String inviteeName,
        ...
        try {
            ...
            // Nếu chấp nhận và có MeetingResponse, gửi email với file .ics đính kèm
            if (isAccepted && meetingResponse != null) {
                try {
                    jakarta.mail.internet.MimeMessage mimeMessage = mailSender.createMimeMessage();
                    ...
                    // Tạo và đính kèm file .ics
                    String icsContent = IcsGenerator.generateIcs(meetingResponse);
                    String fileName = "meeting-invitation.ics";
                    ...
                    // Nếu lỗi khi gửi với attachment, fallback về gửi text thông thường
                    System.err.println("Không thể gửi email với file .ics, gửi email text thông thường: " + e.getMessage());
